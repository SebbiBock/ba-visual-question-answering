...
def kl_divergence(machine_map: np.array, human_map: np.array, epsilon=1e-5):
    ...
    # FLatten
    sm = human_map.flatten()
    fm = machine_map.flatten()
...
def auc_borji(saliency_map, fixation_map, n_rep=100, step_size=0.1):
    """
        ...
        has a value greater or equal than 0.1 to it, meaning that at least one human subject fixated on this point.
...
def nss(saliency_map, fixation_map):
    """
        ...
        greater or equal than 0.1 to it, meaning that at least one human subject fixated on this point.
